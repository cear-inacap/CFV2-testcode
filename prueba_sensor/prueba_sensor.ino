...
unsigned int sensor2_val = 0;
unsigned int sensor3_val = 0;
unsigned int sensor4_val = 0;

unsigned int sensor1_array[100] = {0};
byte measure_counter = 0;
...
void setup() {
    ...
    pinMode(M1_sensor, INPUT);
    pinMode(M2_sensor, INPUT);
    pinMode(M3_sensor, INPUT);
    pinMode(M4_sensor, INPUT);
...
void loop() {
    sensor1_val = analogRead(M1_sensor);
...
int read_sensor(){
    sensor1_val = analogRead(M1_sensor);
    sensor2_val = analogRead(M2_sensor);
    sensor3_val = analogRead(M3_sensor);
    sensor4_val = analogRead(M4_sensor);

    sensor1_array[measure_counter] = sensor1_val;
    measure_counter++;

    unsigned int sensor1_prom = 0;
    if(measure_counter > 50){
        for(int x=0; x<50; x++) sensor1_prom = sensor1_prom + sensor1_array[x];
        sensor1_prom = sensor1_prom / 50;
        measure_counter = 0;
        Serial.println(sensor1_prom);
    }
}
